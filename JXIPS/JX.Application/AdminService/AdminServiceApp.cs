...
namespace JX.Application
{
	...
	public partial class AdminServiceApp : IAdminServiceApp
	{
		...
		/// <returns></returns>
		public virtual AdminDTO GetDTOFull(Expression<Func<AdminDTO, bool>> predicate)
		{
			var entity = _repository.GetEntityFull(GetEntityLamada(predicate));
			return Mapper.Map<AdminDTO>(entity);
		}
		/// <summary>
		/// 获取实体，包括扩展属性（异步方式）
		...
		/// <param name="predicate"></param>
		/// <returns></returns>
		public virtual async Task<AdminDTO> GetDTOFullAsync(Expression<Func<AdminDTO, bool>> predicate)
		{
			var entity = await _repository.GetEntityFullAsync(GetEntityLamada(predicate));
			return Mapper.Map<AdminDTO>(entity);
		}
		...
		/// <param name="predicate"></param>
		/// <returns></returns>
		public virtual IList<AdminDTO> GetDTOFullList(Expression<Func<AdminDTO, bool>> predicate)
		{
			var entity = _repository.GetEntityFullList(GetEntityLamada(predicate));
		...
		/// <param name="predicate"></param>
		/// <returns></returns>
		public virtual async Task<IList<AdminDTO>> GetDTOFullListAsync(Expression<Func<AdminDTO, bool>> predicate)
		{
			var entity = await _repository.GetEntityFullListAsync(GetEntityLamada(predicate));
			return Mapper.Map<IList<AdminDTO>>(entity);
		}
		#endregion
		...
		public async Task<bool> Logout(ClaimsPrincipal claimsPrincipal)
		{
			...
			//得到实体
			Admin admin = await _repository.GetAsync(p=>p.AdminID== DataConverter.CLng(adminID));
			if (admin != null && admin.AdminID > 0)
			{
		...
		public async Task<string> AddAdminFull(AdminDTO adminDTO)
		{
			...
			{
				adminDTO.AdminID = GetMax(p=>p.AdminID)+1;
			}
			if (!Add(adminDTO))
			{
				return "添加管理员失败";
