...
{
	/// <summary>
	/// 后台管理员相关模块的应用层服务接口实现类.(包括：Admin、Roles、AdminRoles、RolesPermissions)
	/// </summary>
	public partial class AdminServiceAppADO : IAdminServiceAppADO
	{
		#region 仓储接口
		private readonly IRolesRepositoryADO _RolesRepository;
		...
		public AdminServiceAppADO(IAdminRepositoryADO repository, 
			IRolesRepositoryADO rolesRepository,
		...
		/// </summary>
		/// <returns></returns>
		public AdminDTO GetDTOFull(System.Int32 adminID)
		{
		...
		/// </summary>
		/// <returns></returns>
		public async Task<AdminDTO> GetDTOFullAsync(System.Int32 adminID)
		{
		...
		/// <returns></returns>
		public virtual AdminDTO GetDTOFull(string strWhere, Dictionary<string, object> dict = null)
		{
			var entity = _repository.GetEntityFull(strWhere, dict);
			return Mapper.Map<AdminDTO>(entity);
		}
		/// <summary>
		/// 获取实体，包括扩展属性（异步方式）
		...
		/// <param name="dict">参数的名/值集合</param>
		/// <returns></returns>
		public virtual async Task<AdminDTO> GetDTOFullAsync(string strWhere, Dictionary<string, object> dict = null)
		{
			var entity = await _repository.GetEntityFullAsync(strWhere, dict);
			return Mapper.Map<AdminDTO>(entity);
		}
		...
		/// <returns></returns>
		public AdminDTO GetAdminFull(AdminDTO admin)
		{
			if (admin == null)
				return admin;
			var entity = _repository.GetAdminFull(Mapper.Map<Admin>(admin));
			return Mapper.Map<AdminDTO>(entity);
		}
		#endregion
		...
		/// <param name="dict">参数的名/值集合</param>
		/// <returns></returns>
		public virtual IList<AdminDTO> GetDTOFullList(string strWhere = "", Dictionary<string, object> dict = null)
		{
			var entity = _repository.GetEntityFullList(strWhere, dict);
			return Mapper.Map<IList<AdminDTO>>(entity);
		}
		/// <summary>
		/// 得到实体列表，包括扩展属性（异步方式）
		/// </summary>
		...
		/// <param name="dict">参数的名/值集合</param>
		/// <returns></returns>
		public virtual async Task<IList<AdminDTO>> GetDTOFullListAsync(string strWhere = "", Dictionary<string, object> dict = null)
		{
			var entity = await _repository.GetEntityFullListAsync(strWhere, dict);
			return Mapper.Map<IList<AdminDTO>>(entity);
		}
		#endregion
		...
		/// <returns></returns>
		public IList<AdminDTO> GetListFull(int startRowIndexId, int maxNumberRows, string Filter, out int Total)
		{
			Total = 0;
			var entityList = _repository.GetList(startRowIndexId, maxNumberRows, Filter, out Total);
		...
		/// <returns></returns>
		public async Task<IdentityResult> Login(string adminName, string adminPassword, string manageCode = "")
		{
			//检查管理认证码
			...
			//得到实体
			Admin admin = await _repository.GetEntityAsync(adminName,adminPassword);
			if (admin == null)
			{
				await _repository.CumulativeLoginErrTimes(adminName);
			...
			await _repository.UpdateLoginErrTimes(0,adminName);
		...
		public async Task<string> ModifyPassword(ClaimsPrincipal claimsPrincipal, string oldPwd, string newPwd)
		{
			...
			if (string.IsNullOrEmpty(newPwd))
				return "新密码不能为空";
			Admin admin = await _repository.GetEntityByAdminNameAsync(claimsPrincipal.FindFirst(ClaimTypes.Name).Value);
			if(admin == null)
				return "管理员不存在";
		...
		/// <returns></returns>
		public async Task<string> AddAdminFull(AdminDTO adminDTO)
		{
			if (adminDTO == null)
				return "没有指定管理员信息，添加失败";
			if (string.IsNullOrEmpty(adminDTO.AdminName))
				return "管理员名称不能为空";
			...
			adminDTO.AdminPassword = StringHelper.MD5(adminDTO.AdminPassword);
			adminDTO.Hash = await _repository.GetAuthenticatePassHash(adminDTO.AdminPassword);
			int id = Insert(adminDTO);
			if (id<=0)
			{
				return "添加管理员失败";
			}
			bool result = false;
			if (StringHelper.FoundCharInArr(adminDTO.RoleIDs, "0"))
			{
				result = await _AdminRolesRepository.AddMemberToRolesAsync(id, "0");
			}
			else
			{
				result = await _AdminRolesRepository.AddMemberToRolesAsync(id, adminDTO.RoleIDs);
			}
			if (result)
			{
				return "ok";
		...
		/// <param name="adminDTO"></param>
		/// <returns></returns>
		public async Task<string> UpdateAdminFull(AdminDTO adminDTO)
		{
			if (adminDTO == null)
			...
			if (StringHelper.FoundCharInArr(adminDTO.RoleIDs, "0"))
			{
				result = await _AdminRolesRepository.AddMemberToRolesAsync(adminDTO.AdminID, "0");
			}
			else
			{
				result = await _AdminRolesRepository.AddMemberToRolesAsync(adminDTO.AdminID, adminDTO.RoleIDs);
			}
			if (result)
			{
				return "ok";
		...
		public bool SetAdminStatus(int id)
		{
			if (id <= 0)
				return false;
			var admin = _repository.GetEntity(id);
			admin.IsLock = !admin.IsLock;
			return _repository.Update(admin);
		}

		#region 辅助方法
		private ClaimsPrincipal CreateClaimsIdentity(Admin admin)
		{
