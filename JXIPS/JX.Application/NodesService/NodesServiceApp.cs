...
namespace JX.Application
{
	...
	public partial class NodesServiceApp : INodesServiceApp
	{
		...
		/// <returns></returns>
		public NodesDTO GetNodeById(int nodeId)
		{
		...
		/// <param name="nodeId"></param>
		/// <returns></returns>
		public NodesDTO GetCacheNodeById(int nodeId)
		{
			if ((nodeId != -2) && (nodeId <= 0))
			{
				return new NodesDTO();
			...
			var nodeById = CacheHelper.CacheServiceProvider.Get<NodesDTO>(key);
			if (nodeById == null)
			{
		...
		/// <returns></returns>
		public NodesDTO GetNodeByIdentifier(string identifier)
		{
		...
		/// <returns></returns>
		public NodesDTO GetNodeBySubDomain(string subDomain)
		{
		...
		/// <param name="nodeId"></param>
		/// <returns></returns>
		public NodesDTO GetParentNodeByNodeId(int parentId, int nodeId)
		{
		...
		/// <returns></returns>
		public NodesDTO GetRootNodeByParentPath(string parentPath)
		{
			if (!string.IsNullOrEmpty(parentPath))
			{
				...
				for (int i = 0; i < strArray.Length; i++)
				{
					...
					if (nodeId > 0)
					{
						NodesDTO cacheNodeById = GetCacheNodeById(nodeId);
		...
		/// <returns></returns>
		public IList<NodesDTO> GetNodeList(NodeType nodeType = NodeType.None, bool isShowArchiving = false, bool isAddAll = false)
		{
			Expression<Func<NodesDTO, bool>> predicate = p => true;
			...
			IList<NodesDTO> nodesList = LoadListAll(predicate, orderByExpression);
			if (isAddAll)
			{
				NodesDTO item = new NodesDTO();
				item.NodeName = "所有栏目";
				item.NodeID = -1;
				item.Depth = 0;
				item.ParentPath = "0";
		...
		/// <returns></returns>
		public IList<NodesDTO> GetNodeList(string nodesId, bool isShowArchiving = false)
		{
			if (!DataValidator.IsValidId(nodesId))
			{
				return new List<NodesDTO>();
			}
			Expression<Func<NodesDTO, bool>> predicate = p => true;
		...
		/// <returns></returns>
		public IList<NodesDTO> GetNodesListByArchiving(bool isShowArchiving)
		{
		...
		/// </summary>
		/// <returns></returns>
		public IList<NodesDTO> GetNodeListByContainer()
		{
		...
		/// <param name="parentId"></param>
		/// <returns></returns>
		public IList<NodesDTO> GetNodesListByParentId(int parentId)
		{
			Expression<Func<NodesDTO, bool>> predicate = p => p.ParentID==parentId;
			SortModelField sortModelField1 = new SortModelField() { SortName = "RootID", IsDESC = false };
			SortModelField sortModelField2 = new SortModelField() { SortName = "OrderSort", IsDESC = false };
			SortModelField[] orderByExpression = new SortModelField[2];
			orderByExpression[0] = sortModelField1;
		...
		/// <returns></returns>
		public IList<NodesDTO> GetNodesListByRootId(int rootId)
		{
			Expression<Func<NodesDTO, bool>> predicate = p => p.RootID==rootId && p.ParentID > 0;
		...
		/// <returns></returns>
		public IList<NodesDTO> GetNodesListInArrChildId(string arrChildId)
		{
		...
		/// <returns></returns>
		public IList<NodesDTO> GetNodesListInParentPath(string parentPath)
		{
		...
		/// <returns></returns>
		public IList<NodesDTO> GetAnonymousNodeId(int groupId, string operateCode)
		{
		...
		/// </summary>
		/// <returns></returns>
		public IList<NodesDTO> GetContentNodeList()
		{
		...
		/// </summary>
		/// <returns></returns>
		public IList<NodesDTO> GetShopNodeList()
		{
			SqlParameter parmNodeType = new SqlParameter("NodeType", NodeType.Container);
		...
		/// <param name="parentId"></param>
		/// <returns></returns>
		public IList<NodesDTO> GetShopNodeListByParentId(int parentId)
		{
		...
		/// <param name="parentID"></param>
		/// <returns></returns>
		public IList<NodesDTO> GetNodeNameByParentID(int parentID)
		{
		...
		/// <param name="isShowArchiving">是否显示归档节点</param>
		/// <returns></returns>
		public IList<NodesDTO> GetNodeNameForContainerItems(bool isShowArchiving=false)
		{
		...
		/// </summary>
		/// <returns></returns>
		public IList<NodesDTO> GetNodeNameForItems()
		{
		...
		/// </summary>
		/// <returns></returns>
		public IList<NodesDTO> GetNodeNameForItemsExceptOutLinks()
		{
			IList<NodesDTO> nodesList = GetNodeList();
			IList<NodesDTO> nodeList = new List<NodesDTO>();
			foreach (NodesDTO info in nodesList)
			{
		...
		/// </summary>
		/// <returns></returns>
		public IList<NodesDTO> GetNodeNameForContent()
		{
			IList<NodesDTO> nodesList = GetContentNodeList();
		...
		/// </summary>
		/// <returns></returns>
		public IList<NodesDTO> GetNodeNameForShop()
		{
			IList<NodesDTO> nodesList = GetShopNodeList();
			return NodeTreeItems(nodesList);
		}
		...
		/// <returns></returns>
		private IList<NodesDTO> NodeTreeItems(IList<NodesDTO> nodeList)
		{
			int index = 0;
			char ch = '\x00a0';
			bool[] flagArray = new bool[50];
			foreach (NodesDTO info in nodeList)
			{
				index = info.Depth;
				if (info.NextID > 0)
				{
		...
		public bool ExistsTargetNodeIdInArrChildId(int targetNodeId, string arrChildId)
		{
			...
			Expression<Func<NodesDTO, bool>> predicate = p => p.NodeID == targetNodeId;
