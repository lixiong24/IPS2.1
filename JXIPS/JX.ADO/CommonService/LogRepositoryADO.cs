...
namespace JX.ADO
{
	...
	public partial class LogRepositoryADO : ILog
	{
		...
		public void SaveLog(string logTitle, string logMsg, string userName = "", LogCategory logCategory = LogCategory.SystemAction, LogPriority logPriority = LogPriority.Normal, string logSource = "")
		{
			...
			string strHeaders = "Request Headers信息：" + MyHttpContext.Current.Request.Headers.ToJson();
			string strForm = "Request Form信息：";
			if (MyHttpContext.Current.Request.Method.ToUpper() == "POST" && MyHttpContext.Current.Request.HasFormContentType)
			{
		...
		private static Log GetLog(LogInfo logInfo)
		{
			Log info = new Log();
		...
		private static LogInfo GetLogInfo(Log log)
		{
